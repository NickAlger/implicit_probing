# implicit_probing — architecture & developer guide

The design/architecture reference for **contributors**. For installation and a quickstart see the
[README](README.md); for how to contribute (dev setup, tests, PRs) see [CONTRIBUTING](CONTRIBUTING.md).
A standalone Python package for **derivative probing of implicitly-defined maps**.
...
- **Run the tests** with `pytest tests/ -q` after an editable install (`pip install -e .`), or without
  installing via `PYTHONPATH=$PWD python -m pytest tests/ -q`. The FEniCS and JAX tests skip
  automatically when their dependency is absent.

## Knowledge routing

- User-facing docs -> `docs/` (`docs/overview.md` is the user-facing tour, built as a Sphinx site).
  Internal status/handoff -> `dev/HANDOFF.md` (where-we-are + next steps); release path ->
...
- **Essential/Dirichlet BCs** — a constraint for the *future* autodiff-hook layer, NOT the symbolic
  ...
  enforce **homogenized** (zeroed-BC-dof) versions.
